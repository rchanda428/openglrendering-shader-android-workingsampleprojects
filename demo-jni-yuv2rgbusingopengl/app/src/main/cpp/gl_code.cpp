...
//    cv::Mat outputReadpixelInMat = cv::Mat(inputInMat.rows,inputInMat.cols,CV_8UC4,readPixels);
    cv::Mat outputReadpixelInMat = cv::Mat(scnh,scnw,CV_8UC4,readPixels);
...
#if 0 //disabling flip as we are doing it shader
...
#endif
    cv::Mat mat_RGBA2YUV_I420;
    cv::cvtColor(outputReadpixelInMat, mat_RGBA2YUV_I420, cv::COLOR_RGBA2YUV_I420);
    cv::imwrite("/storage/emulated/0/opencvTesting/outputReadpixelYUVMat.jpg", mat_RGBA2YUV_I420);
